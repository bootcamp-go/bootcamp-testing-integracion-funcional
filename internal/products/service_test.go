...
func TestStore(t *testing.T) {
	...
	dbMock := StoreMock{Data: []Product{testProduct}}
	myRepo := NewRepository(&dbMock)
	myService := NewService(myRepo)
...
func (m *MockRepository) Store(id int, nombre, tipo string, cantidad int, precio float64) (Product, error) {
...
func (m *MockRepository) Update(id int, name, productType string, count int, price float64) (Product, error) {
	args := m.Called(id, name, productType, count, price)
	return args.Get(0).(Product), args.Error(1)
}

func (m *MockRepository) UpdateName(id int, name string) (Product, error) {
...
func TestServiceCreateErrorOnCreate(t *testing.T) {
	...
	repo := new(MockRepository)
	repo.On("LastID").Return(0, nil)
	repo.On("Store", input.ID, input.Name, input.Type, input.Count, input.Price).Return(Product{}, fmt.Errorf("error on store"))
